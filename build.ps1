...
Write-Host "========================================="
Write-Host "Pre-build Cleanup: Terminating old zombie processes..."
$processesToKill = @("ICEC_Smart_Assistant", "python", "pythonw", "msedge", "chrome", "node")
foreach ($proc in $processesToKill) {
    # Silently attempt to kill processes that might be locking the DLLs
...
# Ensure PyInstaller failure throws an exception
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "PyInstaller exited with an error code."
    }
} catch {
    Write-Host "=================="
    Write-Host "BUILD FAILED! Please check the logs above." -ForegroundColor Red
    Write-Host "Common reasons: Files are still open in OneDrive, or old EXE is running in the background." -ForegroundColor Yellow
    exit 1
}

Write-Host "=================="
Write-Host "Copying editable configs..."
if (Test-Path "dist\ICEC_Smart_Assistant\configs") {
    Remove-Item -Path "dist\ICEC_Smart_Assistant\configs" -Recurse -Force
}
